...
class AODEFunc(nn.Module):
    ...
    def forward(self, t, x, task = 0, round_ = False):
        ...
        if self.time_dependent:
            ...
            t_and_x = torch.cat([t_vec, x], 1)
            # Shape (batch_size, hidden_dim)
            out = self.fc1(t_and_x, task = task, round_ = round_)
        else:
            out = self.fc1(x, task = task, round_ = round_)
        out = self.non_linearity(out)
        out = self.fc2(out, task = task, round_ = round_)
        out = self.non_linearity(out)
        out = self.fc3(out, task = task, round_ = round_)
        return out


class AODEBlock(nn.Module):
    """Solves ODE defined by odefunc.
    ...
    """
    def __init__(self, odefunc, is_conv=False, tol=1e-3, adjoint=False):
        super(AODEBlock, self).__init__()
        self.adjoint = adjoint
        self.is_conv = is_conv
        self.odefunc = odefunc
        self.tol = tol

    def forward(self, x, eval_times=None, task = 0, round_ = False):
        """Solves ODE starting from x.
        ...
        self.odefunc.update_task(task = task, round_ = round_)
...
class AODENet(nn.Module):
    ...
    def __init__(self, data_dim, hidden_dim, output_dim=1,
        ...
        odefunc = AODEFunc(data_dim, hidden_dim, augment_dim,
                          time_dependent, non_linearity)
    ...
    def forward(self, x, return_features=False, task = 0, round_ = False):
        ...
        pred = self.linear_layer(features, task = task, round_ = round_)
        if return_features:
            return features, pred
        return pred
